...
6. static作用是什么？在C和C++中有何区别？
    ...
    - 不能同时用const和static修饰成员函数，在实现const的成员函数的时候为了确保该函数不能修改类的实例的状态，会在函数中添加一个隐式的参数const              this*。但当一个成员为static的时候，该函数是没有this指针的。
...
　　cout << ( str7 == str8 ) << endl;//1指向文字常量区地址相同
 ...
 28. C中变量的存储类型有哪些？
     ...
     - static int a; 静态存储
     - register int i; 频繁使用的变量放在CPU的寄存器上
     - int a=1;
       extern a;
...
10. 覆盖、重载和隐藏的区别？
     ...
     - 隐藏(或叫重写，overwrite，子类改写父类的虚函数，从而屏蔽父类函数)是指派生类函数屏蔽了与其同名的函数，这里仅要求基类和派生类函数同名即可。其他状态同覆盖。可以说隐藏比覆盖涵盖的范围更宽泛，毕竟参数不加限定。
